...
impl CoverageTool {
    pub fn for_language(language: &str) -> Vec<CoverageTool> {
        match language {
            "typescript" | "javascript" => vec![CoverageTool::Vitest, CoverageTool::C8, CoverageTool::Istanbul],
    ...
    pub fn install_command(&self) -> Option<(&'static str, Vec<&'static str>)> {
        match self {
            ...
            CoverageTool::Vitest => Some(("npm", vec!["install", "--save-dev", "@vitest/coverage-v8"])),
    ...
    pub fn coverage_command<'a>(&self, target: &'a str) -> Vec<&'a str> {
        match self {
            CoverageTool::C8 => vec!["c8", "--reporter=json-summary", "--reporter=json", "--reports-dir=./coverage", target],
